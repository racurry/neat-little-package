# Everything Is Wrapped Up In a Neat Little Package

![Sorry if it sounded sarcastic](assets/really-i-mean-that.png)

A collection of Claude Code plugins.

## Set up

```bash
 # Add the marketplace
/plugin marketplace add racurry/neat-little-package

# Browse the plugins
/plugin 

# Install em
/plugin install hello-world@neat-little-package

# Use the thing
/hello
```

## Plugins

- [box-factory](plugins/box-factory/README.md): A toolkit for creating and managing Claude Code components (plugins, agents, skills, hooks)
- [mr-sparkle](plugins/mr-sparkle/README.md): All the things about how to do code — linting, formatting, git & GitHub workflows, docs and scripting conventions
- [spirograph](plugins/spirograph/README.md): 3D-printed home organization (Gridfinity, OpenGrid, etc.)
- [ultrahouse3000](plugins/ultrahouse3000/README.md): Smart homing with Home Assistant

## Look

- [Official Plugin Documentation](https://code.claude.com/docs/en/plugins)
