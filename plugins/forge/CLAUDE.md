...
1. **Load design skills (REQUIRED)** - Use Skill tool to load both skills BEFORE proceeding
   ...
   Use Skill tool: skill="forge:agent-design"
   ```
...
- Load agent-design and forge-architecture skills
...
**Example:** `/forge:add-agent` → agent-writer → agent-design skill
...
1. Load forge-architecture skill
...
2. Load forge-architecture + component-specific skill
3. Fetch official documentation
...
1. Load both forge-architecture AND component-specific design skill
...
1. Load forge-architecture AND relevant design skills
...
Run `/forge:validate-plugin` on:

- Forge itself (self-validation)
- New plugins created by Forge
- Modified components
...
**Critical principle:** Forge uses its own patterns to create itself.
...
5. Run `/forge:review-component` for feedback
6. Fix any issues
