...
Use Skill tool: skill="forge:plugin-design"
```
...
**Critical:** Create component directories at plugin root, NEVER inside `.claude-plugin/`.
...
**WHY:** Each writer agent (skill-writer, agent-writer, slash-command-writer, hooks-writer) loads its own design skill and follows Forge patterns. Creating components directly bypasses critical validation and design guidance.
...
### 10. Validate Forge Compliance (REQUIRED)

**CRITICAL FINAL STEP:** After creating all components, validate against Forge design principles:
...
**Only create directories for requested components:**
...
**Never create empty directories** - only create when components will be added.
